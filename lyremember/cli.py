...
from lyremember.file_importer import import_file, FileImportError
...
@click.option('--title', default=None, help='Song title (overrides value in file)')
@click.option('--artist', default=None, help='Artist name (overrides value in file)')
@click.option('--language', default=None, help='Language code, e.g. en, es, fr (overrides value in file)')
def import_cmd(filepath, title, artist, language):
    ...
    .json JSON object with optional fields: title, artist, language, lyrics.

    Example:

    \b
        lyremember import song.txt --title "My Song" --artist "Me" --language en
        lyremember import song.json
    """
    try:
        file_title, file_artist, file_language, lyrics = import_file(
            filepath, title=title, artist=artist, language=language
        )
    except FileImportError as exc:
        click.echo(f"{Fore.RED}✗ Import failed: {exc}{Style.RESET_ALL}")
        raise SystemExit(1)

    # Prompt for any missing metadata
    if not file_title:
        file_title = click.prompt('Song title')
    if not file_artist:
        file_artist = click.prompt('Artist name')
    if not file_language:
        file_language = click.prompt('Language code (e.g., en, es, fr)')

    song = song_manager.add_song(file_title, file_artist, file_language, lyrics)

    click.echo(f"\n{Fore.GREEN}✓ Song imported successfully!{Style.RESET_ALL}")
    click.echo(f"  ID: {song.id}")
    click.echo(f"  Title: {song.title}")
    click.echo(f"  Artist: {song.artist}")
    click.echo(f"  Language: {song.language}")
...
def practice(song_id, mode, difficulty):
    ...
    click.echo(f"\n{Fore.CYAN}Practicing: {song.title} by {song.artist}{Style.RESET_ALL}")
